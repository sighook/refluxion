...
FLUXIONAttackDeauthAllClients="Disconnect (deauth) all clients"
FLUXIONAttackDeauthSpecificClient="Disconnect (deauth) only specific client"
FLUXIONAttackDeauthEnterMacAddress="Enter MAC address of the client"
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
